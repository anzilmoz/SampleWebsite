...
class ConsentLanguages {
  static langSet = new Set(["AR", "BG", "BS", "CA", "CS", "DA", "DE", "EL", "EN", "ES", "ET", "EU", "FI", "FR", "GL", "HR", "HU", "IT", "JA", "LT", "LV", "MT", "NL", "NO", "PL", "PT", "PT-BR", "RO", "RU", "SK", "SL", "SR", "SR-LATN", "SV", "TR", "ZH", "CY", "UK"]);
...
(() => {
/*!*******************************************!*\
  !*** ./HPXcDgHrQ6lpUe6kuQDxE-original.js ***!
  \*******************************************/
...
function _ckyPrepareHTML(htmlContent) {
  ...
  const keysToReplace = Object.keys(languageDictionary).reduce((prev, curr) => {
    if (ref._ckyStartsWith(curr, `cky_`)) prev[`[${curr}]`] = languageDictionary[curr] || "";
    return prev;
  }, {});
...
function _ckyReplaceKeysInText(hayStack) {
  const replaceKey = (key, value = null, treatment = 1) => {
    ...
    hayStack = ref._ckyReplaceAll(hayStack, processedKey, value === null ? _ckyGetTranslation(key) : value);
  };
...
function _ckyAttachLinkToLastChild(shortCodeKey, placeHolder, linkKey, selector) {
  const readMoreCode = ref._ckyStore._bannerConfig.dataShortCodes.find(code => code.key === shortCodeKey);

  const contentToInsert = `&nbsp;${ref._ckyReplaceAll(readMoreCode.processedHTML, `[${placeHolder}]`, _ckyGetTranslation(placeHolder)).replace(`href="#"`, `href="${_ckyGetTranslation(linkKey)}"`)}`;
  const readMoreElement = document.querySelector(`[data-cky-tag="${selector}"] p:last-child`);
  readMoreElement && readMoreElement.insertAdjacentHTML("beforeend", contentToInsert);
}
...
function _ckyAcceptCookies(choice = "all") {
  ...
  if (!reloadOnAccept) {
    ...
    if (!ref._ckyStore._isPreview) _ckyLogCookies(reloadOnAccept);else location.reload();
  }
...
function _ckyRemoveDeadCookies({
  ...
  for (const {
    ...
    if (cookieToReset) {
      ...
      [domain, "", strippedHost, cleanedHost].map(cookieDomain => ref._ckySetCookie(cookieToReset, "", 0, cookieDomain));
...
async function _ckyFindApplyingRule() {
  try {
    if (window.ckySettings && window.ckySettings.ruleSet) ref.ruleSet = window.ckySettings.ruleSet;else {
      const response = await _ckyRequest(`https://cdn-cookieyes.com/client_data/b10574dc1134b3a7c22023ec/XKeZinRx.json`);
    ...
    if (firstRule.condition === "all") return ref._ckyStore._banners[firstRule.targetBanner];
    await _ckyGeoIP();
...
function _ckyGetAuditTableContentForPage(category, auditTableBase) {
  ...
  const auditTableBasePrepared = auditTableBase.replace(`[cky_preference_{{category_slug}}_title]`, _ckyGetTranslation(`cky_preference_${category.slug}_title`)).replace(`[cky_preference_{{category_slug}}_description]`, _ckyGetTranslation(`cky_preference_${category.slug}_description`));
...
function _ckyReplaceVendorCount(inputHTML) {
  return ref._ckyReplaceAll(inputHTML, "{{count}}", ref._thirdPartyLists[0].sublist.length + ref._thirdPartyLists[1].sublist.length);
}
  ...
  ref._thirdPartyLists.forEach(section => {
    ...
    section.sublist.forEach(itemForSection => {
      ...
      if (itemForSection.legitimateInterestLink) {
        legitimateInterestLink = ref._ckyReplaceAll(vendorItemLegitimateLink, "[LEGITIMATE_INTEREST_LINK]", itemForSection.legitimateInterestLink);
      }
...
async function _ckySetToggleForList(key, {
  consent,
  legitimateInterest,
  sectionChecked
}) {
...
function _ckyOpenIABThirdpartyList(key) {
  if (!["sidebar-left", "sidebar-right"].includes(ref._ckyStore._bannerConfig.bannerPreferenceCenterType)) _ckyFindElement("#ckyIABTabVendor").click();
...
function _ckySetIABOpenCloseListeners(list, keyPreffix, keySuffix) {
  list.forEach(item => {
    ...
    _ckyAttachListener(itemSelector, ({
      ...
      if (ref._ckyStartsWith(id, `${keyPreffix}${item.id}Toggle`) || !_ckyClassToggle(itemSelector, "cky-accordion-active")) return _ckyToggleAriaExpandStatus(accordionButtonSelector, "false");
    ...
    if (item.sublist) {
      ...
      _ckyAttachListener(`#${keyPreffix}${item.id} > .cky-accordion-body`, ({
        ...
        if (ref._ckyStartsWith(target.id, `${itemID}Toggle`) || !_ckyClassToggle(`#${itemID} > .cky-child-accordion-item`, "cky-accordion-active")) return _ckyToggleAriaExpandStatus(accordionButtonSelectorSub, "false");
...
function _ckyAttachReadMore() {
  ...
  if (readMoreButton.status) _ckyAttachLinkToLastChild("cky_readmore", "cky_readmore_text", "cky_readmore_privacyLink", activeLaw === "gdpr" ? "iab-description" : "description");
}
...
function _ckySetShowMoreLess() {
  ...
  const hideButtonContent = `${ref._ckyReplaceAll(hideCode.content.button, "[cky_showless_text]", _ckyGetTranslation("cky_showless_text"))}`;
  const showButtonContent = `${ref._ckyReplaceAll(showCode.content.button, "[cky_showmore_text]", _ckyGetTranslation("cky_showmore_text"))}`;
...
async function _ckyGeoIP() {
  try {
    const geoIPResponse = await _ckyRequest("https://directory.cookieyes.com/api/v1/ip");
...
async function _ckyLogCookies(reloadOnAccept = false) {
  try {
    ...
    data.append("key", "b10574dc1134b3a7c22023ec");
    ...
    data.append("cookie_list_version", "3");
...
function _ckyFindApplicableLanguage(currentLang) {
  if (window.ckySettings && window.ckySettings.documentLang) currentLang = window.ckySettings.documentLang;
...
async function _ckyFetchLanguage(language) {
  ...
  let languageFile = {};
  if ((!languageDictionary || !languageDictionary.setLanguageContent) && ref._ckyStore._bannerConfig.languageMap && ref._ckyStore._bannerConfig.languageMap[language]) languageFile = await _ckyGetLanguageJSON(`https://cdn-cookieyes.com/client_data/b10574dc1134b3a7c22023ec/translations/${ref._ckyStore._bannerConfig.languageMap[language]}.json`);
  let languageFileAuditTable = {};
  if (!languageDictionary || !languageDictionary.setAuditTableContent) languageFileAuditTable = await _ckyGetLanguageJSON(`https://cdn-cookieyes.com/client_data/b10574dc1134b3a7c22023ec/audit-table/${ref._ckyStore._language._supportedMap[language]}.json`);
...
function _ckyIABStatusUpdate(uiStatus = false, skip = false) {
  ...
  ref._cmpAPI.update(activeLaw === "gdpr" ? uiStatus && skip ? "" : ref._ckyStore._tcStringValue : null, uiStatus);
}
...
function _ckySetIABConsentState(choice = "all") {
  ...
  ref._ckySetCookie("euconsent", `${ref._ckyStore._tcStringValue},${ref._ckyEncodeACString(ref._addtlConsent || "")}`, ref._ckyStore._bannerConfig.scriptExpiry);
...
async function _ckySetupBanner(bannerID) {
  const response = await _ckyRequest(`https://cdn-cookieyes.com/client_data/b10574dc1134b3a7c22023ec/config/${bannerID}.json`);
